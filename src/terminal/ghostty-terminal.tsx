...
const GhosttyTerminal = forwardRef<GhosttyTerminalHandle, GhosttyTerminalProps>(
  function GhosttyTerminal({ onWindowCommand }, ref) {
    const containerRef = useRef<HTMLDivElement>(null);
    const termRef = useRef<import("ghostty-web").Terminal | null>(null);
    const cleanupRef = useRef<(() => void) | null>(null);
    const shellRef = useRef<{ closeWindow(id: string): void } | null>(null);
    const onWindowCommandRef = useRef(onWindowCommand);
    onWindowCommandRef.current = onWindowCommand;

    useImperativeHandle(ref, () => ({
      closeWindow(windowId: string) {
        shellRef.current?.closeWindow(windowId);
      },
    }));
    ...
    const initTerminal = useCallback(async () => {
      ...
      const shell = attachShell(term, (cmd) =>
