...
export type { WindowCommand } from "./filesystem";
...
function handleCd(
  ...
  cwd: string,
  args: string[]
): { newCwd: string; result: CommandResult | null } {
  if (args.length === 0 || args[0] === "~" || args[0] === "/") {
    return { newCwd: "~", result: null };
  }
  ...
  return { newCwd: target, result: null };
}
...
function handleTouch(
  ...
  if (fs[path]) {
    // File already exists, silently succeed like real touch
    return { type: "output", text: "" };
  }
  fs[path] = {
    kind: "file",
...
function handleRm(
  ...
  // Remove user-created entry (and children if dir)
  if (entry.kind === "dir") {
    const prefix = `${path}/`;
...
export function attachShell(
  ...
  const onDataDisposable = term.onData((data: string) => {
    if (data === "\r") {
      ...
      if (trimmed) {
        ...
        const result = executeCommand(fs, cwd, cmd, args);

        if (result) {
          if ("newCwd" in result) {
            cwd = result.newCwd;
            if (result.result) writeResult(result.result);
          } else {
            writeResult(result);
          }
        }
  ...
  function writeResult(result: CommandResult) {
    switch (result.type) {
      case "output":
...
type CdResult = { newCwd: string; result: CommandResult | null };
